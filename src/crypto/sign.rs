...
impl KeyPair {
        ...
        Octs: AsRef<[u8]>,
    {
        ...
        #[allow(unused_mut)] // occurs if no other backend is enabled
        #[allow(unused_assignments)] // the default is only read if no
        // other backend is enabled
        let mut error = FromBytesError::UnsupportedAlgorithm;
...
pub fn generate(
    ...
    #[allow(unused_assignments)] // the default is only read if no
    // other backend is enabled
    let mut error = GenerateError::UnsupportedAlgorithm;
